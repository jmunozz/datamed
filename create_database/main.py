...
def load_to_df_cis_cip_bdpm(fpath, _settings):
    args = {**{"filepath_or_buffer": fpath}, **_settings}
    df = pd.read_csv(**args)
    # Cleaning
    df = df.drop(
...
def create_table_cis_cip_bdpm(_settings):
    fpath = find_cis_cip_bdpm(_settings["source"]["pattern"])
    df = load_to_df_cis_cip_bdpm(fpath, _settings["read_csv"])
    db.create_table_from_df(df, _settings["to_sql"])
...
def create_substance_ordei_table(_settings):
    ...
    df_by_years["exposition"] = df_by_years["conso_annee"].apply(
        helpers.get_exposition_level, type="substance"
    )
    df_by_code = df_by_years.groupby("code").agg(
        conso=("conso_annee", "sum"), cas=("cas_annee", "sum")
    )
...
def create_hlt_table(_settings_soclong, _settings):
    ...
    total_case_per_sex_and_age = soclong_df.groupby(["code", "sexe", "age"]).agg(
        {"n_cas": "max"}
    )
