...
def get_api_correspondence(df: pd.DataFrame, api_by_cis: Dict) -> DefaultDict:
    ...
    cis_set = cis_table.intersection(
        cis_rsp
    )  # CIS in both fabrication_sites and COMPO.txt
    cis_not_in_rsp = (
        cis_table - cis_set
    )  # CIS that are in fabrication_sites but not in COMPO.txt
    print(
        "{} CIS codes over {} are not referenced in the RSP".format(
            len(cis_not_in_rsp), len(df.cis.unique())
        ),
        end="\n",
    )
    ...
    return [
        {
            ...
            "cos_sim": api_rsp_dict[
                max(api_rsp_dict, key=api_rsp_dict.get, default=None)
            ],
        }
...
def create_columns(cursor, table_name: str, col_name: str):
    ...
    try:
        ...
        cursor.execute(
            "ALTER TABLE {} ADD {} TEXT NULL AFTER substance_active;".format(
                table_name, col_name
            )
        )
        cursor.execute(
            "ALTER TABLE {} ADD {} FLOAT NULL AFTER {};".format(
                table_name, "cosine_similarity", col_name
            )
        )
        ...
        if e.args[0] == 1060:
            ...
            cursor.execute(
                "ALTER TABLE {} DROP COLUMN {}, DROP COLUMN {};".format(
                    table_name, col_name, "cosine_similarity"
                )
            )
...
def add_best_match_api_to_table(
    best_match_api: List[Dict], table_name: str, col_name: str
):
    """
...
def join_tables(table_name_1: str = "fabrication_sites", table_name_2: str = "atc"):
    ...
    try:
        cursor.execute(
            "ALTER TABLE {} ADD {} TEXT NULL AFTER {};".format(
                table_name_1, "atc", "cis"
            )
        )
    except pymysql.Error as e:
        if e.args[0] == 1060:
            print(e.args[1])
            cursor.execute("ALTER TABLE {} DROP COLUMN {};".format(table_name_1, "atc"))
        else:
            print(e)
