from os import path

P_ROOT = "~/Documents/GitHub/datamed/create_database/data/"

P_VENTES_2017 = path.join(P_ROOT, "OCTAVE/Octave_2017_ATC_voie.xls")
P_VENTES_2018 = path.join(P_ROOT, "OCTAVE/Octave_2018_ATC_voie.xls")
P_VENTES_2019 = path.join(P_ROOT, "OCTAVE/Octave_2019_ATC_voie.xls")

P_CIS_HAS_SMR = path.join(P_ROOT, "CIS_HAS_SMR_bdpm.txt")
P_CIS_HAS_ASMR = path.join(P_ROOT, "CIS_HAS_ASMR_bdpm.txt")

P_CIS_RSP = path.join(P_ROOT, "RSP/CIS_RSP.txt")
P_CIS_BDPM = path.join(P_ROOT, "BDPM/CIS_bdpm.txt")
P_COMPO_RSP = path.join(P_ROOT, "RSP/COMPO_RSP.txt")
P_CIS_CIP_BDPM = path.join(P_ROOT, "BDPM/CIS_CIP_bdpm.txt")
P_CIS_ATC = path.join(P_ROOT, "CIS-ATC_2021-01-04.xlsx")
P_RUPTURES = "analysis/data/decret_stock/ruptures.csv"

P_NOTICE = "./data/notices.json.zip"

P_ORDEI_SPECIALITE = path.join(P_ROOT, "open_medic2014_2018_cis_agg.csv")
P_ORDEI_SUBSTANCE = path.join(P_ROOT, "bnpv_open_medic1418_sa_codex.csv")
P_ORDEI_NOTIF = path.join(P_ROOT, "bnpv_notif_sa_codex_open.csv")
P_ORDEI_SOCLONG = path.join(P_ROOT, "bnpv_eff_soclong_sa_codex_open.csv")
P_ORDEI_HLT = path.join(P_ROOT, "bnpv_eff_hlt_soclong_sa_codex_open.csv")
